...
class DCBOW(nn.Module):
    def __init__(self, vocab_size, embedding_size, hidden_size, tag_size):
        ...
        self.outputLayer = nn.Linear(hidden_size, tag_size, bias=False)
    ...
    def forward(self, x):
        ...
        score1 = F.tanh(self.linear1(embed_score)) # b*h -> b*h
        score2 = F.tanh(self.linear2(score1)) # b*h -> b*h
        word_score = self.outputLayer(score2) # b*h -> b*t
...
# create model
model = DCBOW(nwords, config.embedding_size, config.hidden_size, ntags)
